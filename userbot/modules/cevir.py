...
async def cevir(event):
    ...
    if islem == "foto" or islem == "photo":
        ...
        await event.client.send_file(event.chat_id, "sticker.png", reply_to=rep_msg, caption="@EpicUserBot `ile Fotoğrafa çevirildi`")
        ...
        try:
            await event.client.send_file(event.chat_id, "out.gif",reply_to=rep_msg, caption=LANG['WITH_EPİC_GIF'])
        except:
