...
def register(**args):
    ...
    def decorator(func):
        async def wrapper(check):
            ...
            if ForceVer > EpicVer:
                await check.edit(f"`🌈 Təcili olaraq botu yeniləməlisiniz! Bu versiya artıq mövcud deyil..`\n\n__🥺 Problemi həll etmək__ `.update now` __Yazmalısan!__")
                return
            ...
            if groups_only and not check.is_group:
                await check.respond("`⛔ Bunun bir qrup olduğunu düşünmürəm. Bu plugini bir grupta dene! `")
                return
            ...
            try:
                ...
                if not disable_errors:
                    ...
                    text = "**==USERBOT XƏTA BİLDİRİŞİ==**\n"
                    link = "[Epic Dəstək Qrupuna](https://t.me/HydraDestek)"
                    if len(eventtext)<10:
                        text += f"\n**🗒️ Şu yüzden:** {eventtext}\n"
                    text += "\nℹ️ İstəsəniz bunu bizə bildirə bilərsiniz."
                    text += f"- Sadəcə bu mesajı {link} gönderin.\n"
                    ...
                    if LOGSPAMMER:
                        try:
                            await check.edit("`❕ Üzr istəyirik, UserBot bir səhvlə qarşılaşdı. \n ℹ️ Xəta qeydləri UserBot günlük qrupunda saxlanılır`")
                        except:
                            pass
